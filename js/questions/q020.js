                ...
                <div className="bg-green-50 border border-green-200 rounded-2xl p-5 text-center mb-4">
                    ...
                    <div className="bg-white rounded-xl p-4 text-left text-slate-700 space-y-2 border border-green-100">
                        ${grids.map((grid, idx) => {
                            ...
                            const count = nonZero.reduce((a, b) => a + b, 0);
                            const isOdd = idx === oddPos;
                            return html`
                                <div key=${idx} className=${`flex justify-between items-center text-sm ${isOdd ? 'font-black text-red-600' : 'text-slate-600'}`}>
                                    <span>（${labels[idx]}）</span>
                                    <span>${nonZero.join(' + ')} = ${count} 塊${isOdd ? ` ← 不是 ${targetN} 塊！` : ' ✓'}</span>
                                </div>
                            `;
                        })}
                        <div className="border-t border-green-100 pt-2 flex justify-between items-center">
                            <span className="font-bold">答案：</span>
                            <span className="font-black text-green-700 text-xl">（${labels[oddPos]}）不是 ${targetN} 塊 ✓</span>
                        </div>
                    </div>
                    <button
                        onClick=${newProblem}

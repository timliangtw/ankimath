...
#### 3a-1. 判斷「題組」還是「多個獨立題目」（重要）

一張圖裡若不只一個問題，先分辨它是哪一種：

**A. 題組**（共用同一段題幹／情境／同一張表格、圖表，底下有 (1)(2)(3) 小題）
→ **以題組出題**：整組寫在**同一個** qXXX.js 裡，不要拆成多個檔案。
- 題幹（情境、表格、圖）固定顯示在上方，小題依序作答
- 答對第 1 小題才顯示第 2 小題，全部小題答對才算整題完成
- 有隨機化時，整組小題**共用同一組隨機數字**，換題時整組一起換
- 只有在最後一小題答對後才呼叫換題按鈕；任何一小題答錯都要呼叫 `window.onIncorrectAnswer()`

**B. 多個各自獨立的題目**（彼此沒有共用題幹）
→ 每題各建一個獨立的 qXXX.js。

**C. 拍歪／殘缺的情況（優先於 A、B）**
一張圖裡有兩三個題目，但因為拍歪、被裁切、被遮住，**只有其中一題的「題目文字 + 選項/答案」是完整的**
→ **只做那個完整的題目**，其餘殘缺的直接忽略，**不要靠猜測補齊缺漏的內容**。
在最後的摘要中列出「已略過（畫面不完整）：<簡述>」告知使用者。
...
- 同一張圖有多個題目時，依 3a-1 判斷：題組 → 合成一個 qXXX.js；各自獨立 → 每題一個檔案；只有一題拍得完整 → 只做那一題
- 題目內容一律以圖片上看得到的為準，缺漏的部分不要自行補寫或推測
